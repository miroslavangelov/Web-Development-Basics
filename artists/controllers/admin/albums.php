<?php
	...
	class Albums_Controller extends Admin_Controller{
		public function __construct() {
			parent::__construct(get_class(), "album", "views/admin/albums/");
		}
		...
		public function edit($id) {
			if (!empty($_POST["name"]) && !empty($_POST["year"]) && !empty($_POST["id"])) {
				$name = $_POST["name"];
				$year = $_POST["year"];
				$id = $_POST["id"];
				$album = array(
		...
		public function view($id) {
			...
			if (empty($albums)) die("No album to view here");
			$album = $albums[0];
			$artist_id = $album["artist_id"];
			include DX_ROOT_DIR . "/models/artists.php";
			$artist_model = new \Models\Artists_Model();
